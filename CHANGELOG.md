...
### Added
- **Persisted subagent sessions — resumable across cleanup and `/resume`.** Subagent sessions are now written to disk in pi's standard session JSONL format (proper `session` header + tree-structured message entries) so they survive the manager's normal in-memory cleanup (10-min post-completion timer, `clearCompleted()` on session start/switch). Previously, sessions were created with `SessionManager.inMemory()` and were lost the moment the in-memory reference was dropped — `Agent` with `resume:` would fail with "has no active session" even seconds after a background agent finished. Now `manager.resume()` has two paths: the fast path (live in-memory session, unchanged) and a slow path that rehydrates from disk via `SessionManager.open()` + a freshly-rebuilt `AgentSession` (loader, tools, prompt, model all reconstructed from the agent type — functionally equivalent to a fresh spawn, just with the prior conversation already loaded). Files land at `~/.pi/agent/sessions/--<encoded-cwd>--/subagents/<parent-session-id>/<timestamp>_<sessionId>.jsonl`, mirroring the existing audit-output directory; because pi's session picker only lists files *directly* in `--<encoded-cwd>--/` (never recursing into subdirectories), subagent sessions remain hidden from the main `/sessions` list — verified against `SessionManager.list()` and `SessionManager.listAll()`. The manager captures `session.sessionFile` on the `AgentRecord` (new `sessionFilePath` field) plus a small `configSnapshot` (resolved model key, thinking level, isolated flag, effective cwd, config cwd) so rehydration doesn't need the caller's original options. The agent type's frontmatter stays the source of truth for tools/extensions/skills — if the definition changed since spawn, the rehydrated session picks up the new one (matching how pi itself reloads sessions). On cleanup, only the in-memory `session` reference is dropped (`dispose()` is purely memory cleanup — files are never unlinked); the JSONL stays for rehydration. The audit output file (`<agentId>.jsonl` with `system_snapshot` entries) is kept alongside as before — two files per agent, the pi-format session for reloading, the audit file for debugging/viewing.
